...
def smb_connection(user, pw, svr, port, smbshare, dir):
    logger(0, 'smb://{}:{}@{}:{}/{}'.format(user, 'passwordhere', svr, port, smbshare + dir))
    fs_conn = fs.open_fs('smb://{}:{}@{}:{}/{}?direct-tcp=True&name-port=139&timeout=15&domain='.format(user, pw, svr, port, smbshare + dir))
    # timeout, name - port, direct - tcp, hostname, and domain.
    return fs_conn


def ftps_connection(user, pw, svr, port, tls, dir):
    logger(0, 'ftps host: {} user: {} password: {} port: {} tls: {} dir: {}'.format(svr, user, 'passwordhere', from_port, tls, dir))
...
if from_method == 'smb':
    ...
    from_fs = smb_connection(from_user, from_pw, from_svr, from_port, from_share, from_dir)

if from_method == 'ftps':
    logger(0, 'Create from FTPS connection')
    from_fs = ftps_connection(from_user, from_pw, from_svr, from_port, True, from_dir)
